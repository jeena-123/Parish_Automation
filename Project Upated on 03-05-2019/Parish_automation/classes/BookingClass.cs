...
namespace Parish_automation.classes
{
    public class BookingClass
    {
        ...
        public DataTable CheckAvail()
        {
            ...
            SqlCommand cmd = new SqlCommand("select id from ParishHallBooking where eventdate=@edate and audiname=@eparishhall and is_approved='1'", con);
       ...
       public void InsertBookingDetails()
        {
            OpenConection();
            string qry = "insert into ParishHallBooking(name,address,mobno,emailid,audiname,eventtype,eventdate,noofdays,amount)values(@sname,@saddress,@smobno,@semail,@saudiname,@etype,@edate,@number,@samount)";
