...
namespace Parish_automation.classes
{
    public class register
    {
        ...
        public DataTable DisplyDetails()
        {
            ...
            condition += (type == "1" ? "FI.dob between @datefrom AND @date_to" : string.Empty);
            condition += (type == "2" ? "FI.dateofbaptisam between @datefrom AND @date_to" : string.Empty);
            condition += (type == "3" ? "FI.marriage_date between @datefrom AND @date_to" : string.Empty);
            condition += (type == "4" ? "FI.date_of_death between @datefrom AND @date_to" : string.Empty);
