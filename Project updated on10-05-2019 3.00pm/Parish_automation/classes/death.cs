...
namespace Parish_automation.classes
{
    public class death
    {
        ...
        public DataTable getemail()
        {
            ...
            string qry1 = "select emailid,username,password from Family_Register where family_id=@email";
            SqlCommand cmd = new SqlCommand(qry1, con);
            cmd.Parameters.AddWithValue("@email", updatebutton);
            SqlDataAdapter da = new SqlDataAdapter(cmd);// this will query your database and return the result to your datatable
            da.Fill(dtemail);
            CloseConnection();
            return dtemail;
        ...
        public DataTable DisplyParishRequestDetails()
        {
            OpenConection();
            DataTable dtReg = new DataTable();
            string qry = "select name,address,mobno,emailid,audiname,eventtype,eventdate,noofdays,is_approved, " +
                        " CASE WHEN is_approved=0 THEN 'NOT APPROVED' " +
                        " ELSE 'APPROVED' END AS APR_STATUS " +
        ...
        public void UpdateTable1()
        {
            ...
            SqlCommand cmd = new SqlCommand("update ParishHallBooking  set is_approved='1' where name=@family", con);

            cmd.Parameters.AddWithValue("@family", updatebutton1);
