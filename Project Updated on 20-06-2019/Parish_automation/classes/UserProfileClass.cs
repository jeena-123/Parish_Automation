...
namespace Parish_automation.classes
{
    public class UserProfileClass
    {
        ...
        public string Membertype { get => membertype; set => membertype = value; }
        public string Image { get => image; set => image = value; }
        public string Dueid { get => dueid; set => dueid = value; }

        public DataTable ExecuteSelect()
        {
            OpenConection();
            DataTable dt1 = new DataTable();
            SqlCommand cmd2 = new SqlCommand("select family_head_name,housename,resphoneno,prayergroup,image from Family_Register where family_id=@username", con);
        ...
        {
            OpenConection();
            string qry = "insert into association_register (Member_name,address,mobno,year_from,year_to,emailid,username,password,membertype,approved) values(@name,@address,@mobno,@yearfrom,@yearto,@email,@username,@password,@membertype,@approved)";
            ...
            cmd.Parameters.AddWithValue("@membertype","vicer" );
            ...
            cmd1.Parameters.AddWithValue("@member_type", "vicer");
        ...
        public DataTable ExecuteSelectfamily()
        {
            OpenConection();
        ...
        public DataTable kPaidAmount1()
        {
            OpenConection();
            DataTable dtkpaid = new DataTable();
            SqlCommand cmd = new SqlCommand("select sum(amount) as pamount from ChurchIncomeRegister where familyid=@fid and accountDescription=' KendraVihidham'", con);
            cmd.Parameters.AddWithValue("@fid", dueid);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            da.Fill(dtkpaid);
            CloseConnection();
            return dtkpaid;
        }
        public DataTable mPaidAmount1()
        {
            OpenConection();
            DataTable dtmpaid = new DataTable();
            SqlCommand cmd = new SqlCommand("select sum(amount) as mamount from ChurchIncomeRegister where familyid=@fid and accountDescription='Masavari'", con);
        ...
        public DataTable FetchFixedAmount1()
        {
            OpenConection();
            DataTable dtfixed = new DataTable();
            SqlCommand cmd = new SqlCommand("select Date,kendravihitham,Masavari from Masavari where family_id=@fid", con);
