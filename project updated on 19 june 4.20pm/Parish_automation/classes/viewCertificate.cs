...
namespace Parish_automation.classes
{
    public class viewCertificate
    {
        ...
        public DataTable PrintDeathCertificate()
        {
            OpenConection();
            DataTable dt = new DataTable();
            SqlCommand cmd2 = new SqlCommand("select name,dob,date_of_death,death_reason from  family_information1 where member_id=@member_id", con);
            cmd2.Parameters.AddWithValue("@member_id", updatebutton2);
            SqlDataAdapter da2 = new SqlDataAdapter(cmd2);
            da2.Fill(dt);
            CloseConnection();
        ...
        public DataTable DispalyCertificate()
        {
            ...
            DataTable dtCheck = new DataTable();
            SqlCommand cmd = new SqlCommand("select family_id, name,[address],[mobno],[date] ,[certificate_type],[purpose]  from certificate_Request where family_id =@familyid and  is_approved=@family", con);
